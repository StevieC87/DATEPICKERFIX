...
function getweekNumberISO = (date: string) => {
  
}


function getFirstDateofWeek(weeknumber: number, year: number) {
  //e.g. it's week 1 2024 
  //2. ISO 8601 Week Date System: An international standard that defines the first week of the year as the week containing the first Thursday of the year (which is equivalent to the week containing January 4th).

  //we need to get the first date of that week
  /* let mock1stjan = new Date(year, 0, 1);
  let mock1stjanWEEKDAY = mock1stjan.getDay();
  if (mock1stjanWEEKDAY >= 4) {
    

  } */
  let mock1stjan = new Date(year, 0, 4);
  let mock1stjanWEEKDAY = mock1stjan.getDay();
  if (mock1stjanWEEKDAY >= 4) {
  ...
  export function makearrayofdatesfromWeek(weeknumber: number, year: number, monthnumber: number, monthname: string): any[] {
