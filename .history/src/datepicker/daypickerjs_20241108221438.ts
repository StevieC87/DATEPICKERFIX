...
function getdatesofFirstWeekISO(year: number) {
  ...
  if (fourthjanWEEKDAY == 0) {
    ...
    let firstTuesday = new Date(year -1 , 11, 30);
    let firstWednesday = new Date(year -1 , 11, 31);
    let firstThursday = new Date(year, 0, 1);
    let firstFriday = new Date(year, 0, 2);
    let firstSaturday = new Date(year, 0, 3);
    let firstSunday = new Date(year, 0, 4); */

    array = [firstMonday, firstTuesday, firstWednesday, firstThursday, firstFriday, firstSaturday, firstSunday];
}
  if(fourthjanWEEKDAY == 1) {
    let firstMonday = new Date(year, 0, 4);
    let firstTuesday = new Date(year, 0, 5);
    let firstWednesday = new Date(year, 0, 6);
    let firstThursday = new Date(year, 0, 7);
    let firstFriday = new Date(year, 0, 8);
    let firstSaturday = new Date(year, 0, 9);
    let firstSunday = new Date(year, 0, 10);
  }
